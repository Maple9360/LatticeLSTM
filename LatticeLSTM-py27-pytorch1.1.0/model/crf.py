...
class CRF(nn.Module):
    ...
    def _viterbi_decode(self, feats, mask):
        ...
        #partition_history = torch.cat(partition_history, 0).view(seq_len, batch_size, tag_size).transpose(1,0).contiguous()
        #partition_history = torch.cat(partition_history,0).view(seq_len, batch_size,-1).transpose(1,0).contiguous()
        # 将二维张量转换为三维张量
        #partition_history = [x.unsqueeze(0) for x in partition_history]
        # 将所有三维张量沿着第一维度拼接起来
