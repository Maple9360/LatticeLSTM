...
def batchify_with_label(input_batch_list, gpu, volatile_flag=False):
    ...
    for idx, (seq, biseq, label, seqlen) in enumerate(zip(words, biwords, labels, word_seq_lengths)):
        ...
        #mask[idx, :seqlen] = torch.Tensor([1] * seqlen).type_as(mask)
        mask[idx, :seqlen] = torch.ones(seqlen,dtype=torch.float)
    ...
    #pad_chars = [chars[idx] + torch.zeros((max_seq_len-len(chars[idx]),1),dtype=torch.float) for idx in range(len(chars))]
    #pad_chars = [torch.cat((chars[idx], torch.zeros((max_seq_len-len(chars[idx]), 1), dtype=torch.float)), dim=0) for idx in range(len(chars))]
    chars_tensor = [torch.Tensor(c).long() for c in chars]
    pad_chars = [torch.cat((c, torch.zeros((max_seq_len-c.shape[0], 1)).long()), dim=0) for c in chars_tensor]
